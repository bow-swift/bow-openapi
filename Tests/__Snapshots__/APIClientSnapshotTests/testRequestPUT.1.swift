...
public protocol DefaultAPI {
    func _putPet() -> EnvIO<API.Config, API.HTTPError, NoResponse>
}
...
extension DefaultAPI {
     ...
     */
    public func putPet() -> EnvIO<API.Config, API.HTTPError, NoResponse> {
        _putPet()
    }
}
...
public class DefaultAPIClient: DefaultAPI {
    ...
    public func _putPet() -> EnvIO<API.Config, API.HTTPError, NoResponse> {
        return EnvIO { apiConfig in
            ...
            guard let url = components?.url ?? URL(string: path) else {
                let data = "DefaultAPI.putPet.URL".data(using: .utf8)!
