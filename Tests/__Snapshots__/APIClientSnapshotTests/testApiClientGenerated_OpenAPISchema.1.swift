...
public protocol DefaultAPI {
    func _testSchemaOpenAPI() -> EnvIO<API.Config, API.HTTPError, NoResponse>
}
...
extension DefaultAPI {
     ...
     */
    public func testSchemaOpenAPI() -> EnvIO<API.Config, API.HTTPError, NoResponse> {
        _testSchemaOpenAPI()
    }
}
...
public class DefaultAPIClient: DefaultAPI {
    ...
    public func _testSchemaOpenAPI() -> EnvIO<API.Config, API.HTTPError, NoResponse> {
        return EnvIO { apiConfig in
            ...
            guard let url = components?.url ?? URL(string: path) else {
                let data = "DefaultAPI.testSchemaOpenAPI.URL".data(using: .utf8)!
