...
func main() {
    ...
    guard FileManager.default.fileExists(atPath: arguments.schema) else { Console.exit(failure: "received invalid scheme path") }
    ...
    APIClient.bow(scheme: arguments.schema, output: arguments.output)
             ...
             .unsafeRunSyncEither()
             .mapLeft { apiError in "could not generate api client for scheme '\(arguments.schema)'\ninformation: \(apiError)" }
